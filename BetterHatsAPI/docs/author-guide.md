...
这个菜单的设计意图是为了让玩家能够更好的了解您在数据包中所做的更改，而不是用于进行可视化的数据编辑。在未来的版本中，不会新增任何能够通过这一菜单来修改数据包内容的功能。别忘了在您的模组描述中告诉用户这个菜单的作用！
...
**提示**：
- 若必须使用 `TickUpdated`，请尽量考虑使用 C# 代码配合 API 来设置自定义条件或事件，相比于在 JSON 中设置字符串并让游戏进行解析，C# 代码能够避免这种不必要的开销。
- 若指定了条件和事件，但未指定触发器时，其默认为 `None`，即仅在戴上帽子时检查条件或执行事件。
...
在自定义了某些条件和事件的情况下，若您未在数据包中使用 `ConditionDescription` 或 `ActionDescription` 字段，BHA 将会在图鉴菜单中告知这一情况，提示您可能忘记了设置描述。若出于某些需求，您确实希望隐藏这些条件或事件的描述，您可以使用 `$Hide` 占位符：
...
```json
{
  "(H)64": {
    ...
    "Condition": "$CustomCondition",
    "Trigger": "DayStarted",
    "Description": "强大的力量等待你去探索。",
    "ConditionDescription": "$Hide"
  }
}
```

将 `ConditionDescription` 或 `ActionDescription` 设置为 `$Hide` 后，BHA 将隐藏该条件或事件的描述，同时不再显示它们的缺失。
...
```json
{
  "description.YourHat": "在这里简单描述您帽子的效果。",
  "condition.YourHat": "描述条件何时激活。",
  "action.YourHat": "描述事件的效果。",
