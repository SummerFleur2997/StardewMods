...
{
    private static void RegisterPatchForSeasonalHats(Harmony harmony)
    {
        try
        {
            var original = AccessTools.Method(typeof(Bush), nameof(Bush.shake));
            var transpiler = AccessTools.Method(
                typeof(HatWithPatches), nameof(Patch_SeasonalHats_shake));
            harmony.Patch(original, transpiler: new HarmonyMethod(transpiler));
            ModEntry.Log("Patched Bush.shake for butterfly bow successfully.");
        }
        catch (Exception ex)
        {
            ModEntry.Log($"Failed to patch for butterfly bow: {ex.Message}", LogLevel.Error);
        }
    }
    ...
    /// </summary>
    private static IEnumerable<CodeInstruction> Patch_SeasonalHats_shake(IEnumerable<CodeInstruction> ci)
    {
        var matcher = new CodeMatcher(ci);

        // Find an anchor instruction for the injection
        var target = new CodeMatch(OpCodes.Callvirt, AccessTools.Method(typeof(Farmer), "get_ForagingLevel"));
        matcher.MatchStartForward(target).Advance(4);
